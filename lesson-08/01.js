...
for (let i = 0; i < PETS.length; i++) {  
    ...
    // Добавляем обработчик события клика на созданную кнопку  
    petButtonElement.addEventListener("click", function(event) {  
        if (cart.length < 3) {  
            cart.push(pet.id);  
            updateCartDisplay();  
            messageBox.textContent = ""; // Сброс сообщения при добавлении  
        } else {  
            messageBox.textContent = "Вы не можете добавить более 3 питомцев"; // Вывод сообщения  
        }  
    });  
...
clearCartButton.addEventListener('click', function () {  
    cart.length = 0;  
    updateCartDisplay();  
    messageBox.textContent = ""; // Очищаем сообщение при очистке корзины  
});
